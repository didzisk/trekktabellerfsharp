module internal Skattetabell2015internal
...
let ``WS-KLASSE-FRADRAG`` klasseSkatteProsent =
    let ``KLASSE1-VANLIG``   = 50400M
    let ``KLASSE2-VANLIG``   = 74250M
    let ``KLASSE1-FINNMARK`` = 65900M
    let ``KLASSE2-FINNMARK`` = 89750M
...
let ``TOPPSKATT-LAV`` klasse = 
    match klasse with
    | KLASSE.KLASSE1     -> 550550M
    | KLASSE.KLASSE2 | _ -> 550550M

let ``TOPPSKATT-HOY`` klasse = 
    match klasse with
    | KLASSE.KLASSE1     -> 885600M
    | KLASSE.KLASSE2 | _ -> 885600M

   (*-----------------------------------------------------------------
    * KOMMUNE/FYLKE - OG FELLES-SKATT
    *-----------------------------------------------------------------*)
let ``FELLES-SKATT-VANLIG``   = 13.15M
let ``FELLES-SKATT-FINNMARK`` =  9.65M
let SKATTORE                  = 13.85M
...
let ``MAX-ANV-MINSTE-FRADRAG``        = 78364M
let ``MAX-ANV-MINSTE-FRADRAG-PENSJ``  = 63536M
(*-----------------------------------------------------------------
    * MINSTEFRADRAG
    *-----------------------------------------------------------------*)
let ``MINSTE-FRAD-PROSENT``      = 43M
let ``MIN-MINSTE-FRADRAG``       = 4000M
let ``MAX-MINSTE-FRADRAG``       = 89050M
(*-----------------------------------------------------------------
    * LØNNSFRADRAG
    *-----------------------------------------------------------------*)
